...
def scan_and_push(TELEGRAM_API_KEY, DEBUGGING_CHAT_ID):
    ...
    for gp in config["telegram"]["groups"]:
        ...
        houses_in_cities = scrape(
            cities=cities,
            apikey=TELEGRAM_API_KEY,
            debug_chat_id=DEBUGGING_CHAT_ID
        )
...
def main():
    ...
    create_table()
    scan_and_push(TELEGRAM_API_KEY, DEBUGGING_CHAT_ID)  # 先跑一次
    schedule.every(3).seconds.do(lambda: scan_and_push(TELEGRAM_API_KEY, DEBUGGING_CHAT_ID))  # 每3秒一次
